/**
 * e3-h1-smoke.mjs — Arnés E2E H1: engine runtime Phaser (canvas) en GameHost.
 *
 * Uso: node e2e/e3-h1-smoke.mjs <ASSIGNMENT_ID>
 *
 * Requiere:
 *   - MongoDB corriendo con seed ejecutado (B2_BUNDLE_URL apuntando a demo-engine-h1).
 ...
 * El hook simula 10 rondas disparando callbacks del sessionState directamente.
 */
...
async function main() {
  log('=== Arnes E3-H1 iniciando. AssignmentId: ' + ASSIGNMENT_ID + ' ===');
  ...
  try {
    await page.waitForSelector('iframe[src*="demo-engine-h1"]', { timeout: 15000 });
    log('Iframe del engine detectado (src contiene demo-engine-h1).');
    iframeHandle = await page.$('iframe[src*="demo-engine-h1"]');
  } catch (_) {
  ...
  console.log('');
  console.log('============================================================');
  console.log('VEREDICTO ARNES E3-H1: ' + (allGreen ? 'VERDE' : 'ROJO'));
