    ...
    return (
      <div className="p-6 flex flex-col items-center text-center h-full justify-center">
        <p className="font-body text-sm text-text-soft leading-relaxed">
          No tienes conversaciones activas. Vincula un profesional para habilitar los mensajes.
        </p>
      </div>
    );
  }
  ...
  return (
    <ul>
      {conversaciones.map((conv) => {
        ...
        const proNombre = conv.invitation.professionalId?.name || conv.invitation.tutorId?.name || 'Profesional';
        return (
          <li key={conv.invitation._id}>
            ...
            >
              <div className="flex items-start gap-3">
                <div className="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center font-heading font-semibold text-primary text-sm flex-shrink-0">
                  {proNombre.charAt(0)}
                </div>
...
function HiloMensajes({ convSeleccionada, userId }) {
  ...
  const proNombre = convSeleccionada.invitation.professionalId?.name
    || convSeleccionada.invitation.tutorId?.name
    || 'Profesional';
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      ...
      <div className="px-6 py-4 border-b border-surface bg-white flex items-center gap-3 flex-shrink-0">
        <div className="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center font-heading font-semibold text-primary text-sm">
          {proNombre.charAt(0)}
        </div>
        <div>
          <p className="font-heading text-sm font-semibold text-text-strong">{proNombre}</p>
...
export default function MensajesTutor() {
  const { user } = useAuth();
